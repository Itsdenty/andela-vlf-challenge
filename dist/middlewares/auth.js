...
JWT.verifyToken = function (req, res, next) {
  ...
  if (!token) {
    return res.status(403).json(_transformer2.default.transformResponse(403, 'No token provided'));
  }

  _jsonwebtoken2.default.verify(token, process.env.JWT_SECRET, function (error, decoded) {
    if (error) {
      return res.status(401).json(_transformer2.default.transformResponse(401, error.message));
    }
...
JWT.isAdmin = function (req, res, next) {
  if (!req.decodedToken.isadmin) {
    return res.status(403).json(_transformer2.default.transformResponse(403, 'only an admin is authorized to access this endpoint'));
  }
  next();
};
